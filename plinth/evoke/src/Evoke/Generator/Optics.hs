...
makePrismDecls srcSpan type_ lens constructor = do
  ...
  let -- Focus type: () / T / (T0, T1, ...)
      fieldTys = fmap (\f -> Ghc.L (Ghc.noAnnSrcSpan srcSpan) (Field.type_ f)) fields
      focusTy = case arity of
        0 -> unitTy srcSpan
        1 -> head fieldTys
        _ -> Ghc.L (Ghc.noAnnSrcSpan srcSpan) $ Ghc.HsTupleTy Ghc.noAnn Ghc.HsBoxedOrConstraintTuple fieldTys
      ...
      valDecl =
        Ghc.noLocA $ Ghc.ValD Ghc.noExtField $
          Ghc.FunBind Ghc.noExtField prismId
            ( Ghc.MG Ghc.Generated
                ( Ghc.L (Ghc.noAnnSrcSpan srcSpan)
                    [ Ghc.L (Ghc.noAnnSrcSpan srcSpan) $
                        Ghc.Match Ghc.noAnn
                          (Ghc.FunRhs prismId Ghc.Prefix Ghc.NoSrcStrict)
                          []
                          (Common.makeGRHSs srcSpan prismExpr)
                    ]
                )
            )
  ...
  _ ->
    let tuplePat =
     ...
     in Hs.lam srcSpan . Hs.mg $
          Ghc.L srcSpan
            [Hs.match srcSpan Ghc.LambdaExpr [tuplePat] (Common.makeGRHSs srcSpan body)]
...
mkMatcher ::
  ...
  Int ->
  Ghc.LHsExpr Ghc.GhcPs
mkMatcher srcSpan lConId vars scrutVar arity =
  let successResult = case arity of
        0 ->
          Hs.app srcSpan
        ...
        1 ->
          Hs.app srcSpan
            (Hs.var srcSpan (Hs.unqual srcSpan (Ghc.mkDataOcc "Just")))
            (Hs.par srcSpan $ Hs.var srcSpan (head vars))
        _ ->
          Hs.app srcSpan
            (Hs.var srcSpan (Hs.unqual srcSpan (Ghc.mkDataOcc "Just")))
            ( Hs.par srcSpan $
      ...
      conMatch =
        Ghc.L (Ghc.noAnnSrcSpan srcSpan) $
          Ghc.Match Ghc.noAnn Ghc.CaseAlt [conPat] (Common.makeGRHSs srcSpan successResult)

      wildMatch =
        Ghc.L (Ghc.noAnnSrcSpan srcSpan) $
          Ghc.Match Ghc.noAnn Ghc.CaseAlt [wildPat] (Common.makeGRHSs srcSpan nothingExpr)

      caseExpr =
        Ghc.L (Ghc.noAnnSrcSpan srcSpan) $
          Ghc.HsCase Ghc.noAnn
            (Hs.var srcSpan scrutVar)
            (Ghc.MG Ghc.Generated (Ghc.L (Ghc.noAnnSrcSpan srcSpan) [conMatch, wildMatch]))
   in Hs.lam srcSpan . Hs.mg $
        Ghc.L srcSpan
...
mkOuterTy srcSpan type_ =
  let loc = Ghc.noAnnSrcSpan srcSpan
      initial = Ghc.L loc $ Ghc.HsTyVar Ghc.noAnn Ghc.NotPromoted (Ghc.L loc $ Type.name type_)
      applied =
        List.foldl'
          ( \acc v ->
              Ghc.L loc $
                Ghc.HsAppTy Ghc.noExtField acc
                  (Ghc.L loc $ Ghc.HsTyVar Ghc.noAnn Ghc.NotPromoted (Ghc.L loc v))
          )
          initial
          (Type.variables type_)
   in case Type.variables type_ of
        [] -> applied
        _ -> Ghc.L loc $ Ghc.HsParTy Ghc.noAnn applied
